...
if ( sizeof($expectedItems) > 0 ) {
    ...
    echo '<div class="upcomingDefault centerBoxWrapper"><h2 class="centerBoxHeading">' . sprintf(TABLE_HEADING_UPCOMING_PRODUCTS, strftime('%B')) . '</h2><ul class="prod-list">';
	...
	for($i=0; $i < sizeof($expectedItems); $i++) {
        ...
        $products_butt2 = '<a class="btn products-button add-to-cart" href="' . zen_href_link($_GET["main_page"], zen_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $expectedItems[$i]["products_id"]) . '">' . zen_image_button(BUTTON_IMAGE_ADD_TO_CART, BUTTON_IN_CART_ALT) . '</a>';
