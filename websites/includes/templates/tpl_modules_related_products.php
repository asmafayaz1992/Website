			...
			<?php
				...
				// show only when 1 or more and equal to or greater than minimum set in admin
					...
					while (!$relatedResult->EOF)
					{
					 ...
					 $list_box_contents[$row][$col] = array('params' => 'class="col-xs-6 col-sm-2"',

					  'text' => (($relatedResult->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<div class="product-col"><a href="' . zen_href_link(zen_get_info_page($relatedResult->fields['products_id']), 'products_id=' . $relatedResult->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $relatedResult->fields['products_image'], $relatedResult->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>') . '<div class="prod-info"><a class="product-name name" href="' . zen_href_link(zen_get_info_page($relatedResult->fields['products_id']), 'products_id=' . $relatedResult->fields['products_id']) . '">' . $relatedResult->fields['products_name'] . '</a></div></div>');
