...
if ($num_products_count > 0) {
  ...
  while (!$specials_index->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsSpecials centeredContent span4"' . ' ' . '',
  ...
  if ($specials_index->RecordCount() > 0) {
    $title = '<div class="alert alert-info"><h2 class="centerBoxHeading">' . sprintf(TABLE_HEADING_SPECIALS_INDEX, strftime('%B')) . '</h2></div>';
