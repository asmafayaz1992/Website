...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsNew centeredContent span4"' . ' ' . '',
  ...
  if ($new_products->RecordCount() > 0) {
    if (isset($new_products_category_id) && $new_products_category_id != 0) {
      ...
      $title = '<div class="alert alert-info"><h2 class="centerBoxHeading">' . sprintf(TABLE_HEADING_NEW_PRODUCTS, strftime('%B')) . ($category_title != '' ? ' - ' . $category_title : '' ) . '</h2></div>';
    } else {
      $title = '<div class="alert alert-info"><h2 class="centerBoxHeading">' . sprintf(TABLE_HEADING_NEW_PRODUCTS, strftime('%B')) . '</h2></div>';
    }
