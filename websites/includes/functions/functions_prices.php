...
// show case only
    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          ...
          $show_sale_discount = '<span class="productPriceDiscount">-' .  $show_discount_amount .  '%</span>';
    ...
    if ($display_special_price) {
      ...
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          $show_sale_price = '<span class="productSalePrice">' . '<s>' . $currencies->display_price($display_sale_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</s>' . '</span>';
        } else {
      ...
      if ($display_sale_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_normal_price = '<span class="productSalePrice">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span>';
        }
        $show_special_price = '';
        $show_sale_price = '';
      }
    ...
    if ($display_normal_price == 0) {
      ...
      $final_display_price = $show_sale_price . $show_special_price . $show_normal_price . $show_sale_discount;
    }
